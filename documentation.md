...
def _front_samples(scan):
    ...
    front_samples = []
    sample_span = max(1, len(ordered_indices) - 1)  # just making sure we don't divide by 0 lol
...
# cleans up a single lidar reading. if it's None/nan/zero/negative, that basically
...
# its (cleaned) distance. Basically, "hey what's the distance at roughly this bearing"
...
# (distance > threshold), basically finding the biggest gap/hallway in front of us.
...
def _largest_open_run(samples, threshold):
    ...
    for sample_angle, sample_distance in samples:
        ...
        if sample_distance > threshold:
            ...
            current_run = []
    # gotta check this in case the run goes all the way to the end of the array
...
def wall_following():
    ...
    # distance right at the wheels basically, used to catch us hugging a wall too hard
...
_largest_open_run(front_samples, OPEN_DISTANCE_THRESHOLD)
    ...
    # (standard PD deriv is (target - last) / delta) might wanna double check the math here
